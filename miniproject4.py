...
class LinkedList:
    ...
    def tambah_di_awal(self, data):
        new_node = Node(data)
        if self.cari_node(data.id_produk) is not None: 
...
# Membuat menu searching dengan jump search
def jump_search_by_id(linked_list, id_produk):
    current_node = linked_list.head
    prev = 0
    while current_node:
        if current_node.data.id_produk == id_produk:
            return current_node.data, prev
        current_node = current_node.next
        prev += 1
    return None, -1

def jump_search_by_name(linked_list, nama_produk):
    current_node = linked_list.head
    prev = 0
    while current_node:
        if nama_produk.lower() in current_node.data.nama_produk.lower():
            return current_node.data, prev
        current_node = current_node.next
        prev += 1
    return None, -1
...
def print_product_table(product):
    table = PrettyTable(["ID Produk", "Nama", "Harga", "Stok", "Rating"])
    table.add_row([product.id_produk, product.nama_produk, product.harga, product.stok, product.rating])
    print(table)
...
==================================================\n''')
        ...
        if not choice_tambah:
            ...
            # Memeriksa apakah ID produk sudah ada
            if manajemen_produk.linked_list.cari_node(id_produk) is not None:
                input("\nID produk sudah ada. Tekan ENTER untuk kembali.")
                os.system("cls")
                continue
...
==================================================\n''')
        if not manajemen_produk.linked_list.head:
            input("Tidak ada produk yang dapat dicari.")
            os.system("cls")
        else:
            print("1. Mencari Produk berdasarkan ID")
            print("2. Mencari Produk berdasarkan Nama Produk")
            search_choice = input("Pilih jenis pencarian (1/2, ENTER untuk skip): ").strip()
            if search_choice == "1":
                id_produk = int(input("Masukkan ID produk yang ingin dicari: "))
                found_produk, steps = jump_search_by_id(manajemen_produk.linked_list, id_produk)
            elif search_choice == "2":
                nama_produk = input("Masukkan nama produk yang ingin dicari: ")
                found_produk, steps = jump_search_by_name(manajemen_produk.linked_list, nama_produk)
            elif search_choice == "":
                os.system("cls")
                continue
            else:
            ...
            if found_produk:
                print("\nProduk ditemukan:")
                print_product_table(found_produk)
            else:
